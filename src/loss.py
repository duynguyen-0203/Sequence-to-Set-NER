...
from src.criterion import HugarianMatcher, Criterion
...
class ModelLoss(Loss):
  def __init__(self, entity_types, device, model, optimizer, scheduler, max_grad_norm):
    ...
    matcher = HugarianMatcher(cost_class=2.0, cost_span=2.0)
  ...
  def compute(self, entity_logits, entity_boundary, entity_types, entity_spans_token, entity_masks):
    outputs = {'pred_logits': entity_logits, 'pred_boundaries': entity_boundary}
